...
fetch("https://smuhammetulas.com/api/User/GetUser").then(response => response.json()).then(data => {
    var allData = data.filter(x => x.id == id)
...
$(document).on('click', '.gonder', function () {
    var yeniSifre = sha256($('#yeniSifre').val());
    var eskiSifre = sha256($('#eskiSifre').val());
    if (eskiSifre == password) {
        if (yeniSifre != password && $('#yeniSifre').val() == $('#yeniSifre2').val()) {
